<template>
    <div class="admin-column">
        <div class="admin-row">
            <input type="button" value="Add new artist" v-on:click="onAddArtist"/>
            ...
            <span v-else>
                ...
                <select class="upload_artist" v-model="$state.selected_artist">
                    <option v-for="artist in artists" v-bind:key="artist.key" v-bind:value="{key: artist.key, label: artist.label}">
                ...
                <input type="file" multiple accept="image/apng, image/avif, image/gif, image/jpeg, image/png, image/svg+xml, image/webp"      
                    v-on:change="onUploadArtwork"
                    :disabled="!$state.selected_artist"
                     v-on:click="value=null;"
                />
            </span>
        </div>
        <div class="admin-row">
            <span>Rewards left: {{balance_reward}}</span>&nbsp;&nbsp;&nbsp;
            <input type="button" value="Add Rewards" v-on:click="onAddRewards"/>
            <input type="button" value="Show Stats" v-on:click="onStats" :disabled="pending_artworks != 0"/>
